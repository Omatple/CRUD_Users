...
if (!$id || !$user = UserEntity::fetchUserById($id)) {
    Navigator::redirect("users.php");
}

if (isset($_POST["resetDefaultImage"])) {
    if (basename($user["image"]) !== ImageConfig::DEFAULT_IMAGE_FILENAME) {
        UserEntity::resetDefaultImage($id);
        ImageHandler::deleteImage($user["image"]);
        $user["image"] = "img/" . ImageConfig::DEFAULT_IMAGE_FILENAME;
    }
    ...
    if (!$hasErrors && !UserValidator::isUniqueField("username", $username, $id)) $hasErrors = true;
    if (!UserValidator::validateEmail($email)) $hasErrors = true;
    if (!$hasErrors && !UserValidator::isUniqueField("email", $email, $id)) $hasErrors = true;
    ...
    if ($hasErrors) {
        Navigator::redirect($_SERVER["PHP_SELF"] . "?id=$id");
    }
    ...
    if (basename($user["image"]) !== basename($imagePath)) {
        ImageHandler::deleteImage($user["image"]);
    }
...
<body class="bg-gray-50 dark:bg-gray-900 antialiased">
    <section class="bg-white dark:bg-gray-900">
        <div class="px-4 mx-auto max-w-2xl lg:py-16">
            <h2 class="mb-4 text-2xl font-bold text-gray-900 dark:text-white">Edit User</h2>
            <form action="<?= htmlspecialchars($_SERVER["PHP_SELF"] . "?id=$id") ?>" method="POST" enctype="multipart/form-data" novalidate>
                <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
                    <div class="sm:col-span-2">
                        <label for="email" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Email</label>
                        <input type="email" name="email" id="email" value="<?= htmlspecialchars($user['email']) ?>" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white" placeholder="User email" required>
                        <?= ErrorDisplay::showError("email") ?>
                    </div>
                    <div class="w-full">
                        <label for="username" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Username</label>
                        <input type="text" name="username" id="username" value="<?= htmlspecialchars($user['username']) ?>" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white" placeholder="User username" required>
                    ...
                    <div>
                        ...
                        <select id="stateId" name="stateId" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white" required>
                            ...
                            <?php foreach (StateEntity::fetchStates() as $state): ?>
                                <option value="<?= htmlspecialchars($state['id']) ?>" <?= ($state["id"] === $user["state_id"]) ? 'selected' : '' ?>><?= htmlspecialchars($state['name']) ?></option>
                            <?php endforeach ?>
                        </select>
                    ...
                    <div>
                        <div class="flex items-center space-x-6">
                            <div class="shrink-0">
                                <img id="preview_img" class="h-16 w-16 object-cover rounded-full" src="<?= htmlspecialchars($user["image"]) ?>" alt="Current profile photo">
                            </div>
                            <label class="block">
                ...
                <div class="mt-12">
                    <button type="submit" class="inline-flex items-center px-5 py-2.5 text-sm font-medium text-white bg-green-700 rounded-lg hover:bg-green-800"><i class="fa-solid fa-user-pen mr-2"></i>Edit User</button>
                    <button type="reset" class="inline-flex items-center px-5 py-2.5 text-sm font-medium text-white bg-yellow-700 rounded-lg hover:bg-yellow-800 ml-4"><i class="fa-regular fa-window-restore mr-2"></i>Reset</button>
                    <a href="users.php" class="inline-flex items-center px-5 py-2.5 text-sm font-medium text-white bg-red-700 rounded-lg hover:bg-red-800 ml-4"><i class="fa-solid fa-backward mr-2"></i>Back</a>
                </div>
            </form>
        </div>
    </section>
